...
- URL: `https://browser-game-replay-capsule.sociobot.in/demo` (or `https://browser-game-replay-capsule.sociobot.in/?demo=1`, which canonically opens `/demo`).
...
- The sample is a deterministic capsule for seed `RC-SAMPLE-FAULT-17`. It contains one normalized pointer input and a `fault-contact` checkpoint. Phone first view shows the seed, event/checkpoint summary, and **Replay sample** before the canvas.
